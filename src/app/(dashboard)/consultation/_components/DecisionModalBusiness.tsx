...
import { DecisionTypes } from "@/types/ConsultationCompanyTypes";
...
export const DecisionModalBusiness = ({ decisions, onClose }: DecisionModalProps) => {
              ...
              {decisions.map((d, i) => (
                <div key={i} className="flex items-start gap-3 p-4 hover:bg-gray-50 transition">
                  ...
                  <div className="flex-1">
                    <p className="text-gray-800">{d.DecisionContent}</p>
                    <p className="text-sm text-gray-500 mt-1">{formatDateBR(d.DecisionDate)}</p>
                  </div>
                </div>
              ))}
            </div>
